        ...
        <div className="mx-auto max-w-6xl">
          ...
          <section className="mt-5 overflow-hidden rounded-xl border border-brand-border bg-brand-surface">
            <div className="flex items-center gap-2.5 border-b border-brand-border px-4 py-3 sm:px-5">
              <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-brand-secondary text-white"><FileClock className="h-4 w-4" /></span>
              <div>
            ...
            <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-end sm:p-5">
              ...
              <button type="button" onClick={refreshAuditHistory} disabled={tasksQuery.isFetching || logsQuery.isFetching} className="flex h-10 shrink-0 items-center justify-center gap-1.5 rounded-md border border-brand-border bg-brand-surface px-3 text-xs font-medium text-brand-secondary transition-colors hover:bg-brand-tertiary disabled:cursor-not-allowed disabled:opacity-50" title="refresh audit history">
                <RefreshCw className={`h-3.5 w-3.5 text-brand-primary ${tasksQuery.isFetching || logsQuery.isFetching ? "animate-spin" : ""}`} />
                <span>Refresh</span>
              </button>
            </div>
                  ...
                  return <li key={log.id} className="relative">
                    ...
                    <div className="rounded-lg border border-brand-border bg-brand-tertiary/35 p-3.5 transition-colors hover:bg-brand-tertiary/60 sm:p-4">
                      <div className="flex flex-col gap-2.5 sm:flex-row sm:items-start sm:justify-between">
                        <div><div className="flex flex-wrap items-center gap-2"><h3 className="text-sm font-semibold text-brand-secondary">{log.event}</h3><span className={`rounded-full border px-2 py-0.5 text-[11px] font-medium ${tone.badge}`}>{tone.label}</span></div>{log.remark && <p className="mt-2 text-sm leading-5 text-brand-secondary">{log.remark}</p>}</div>
